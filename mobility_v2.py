...
def update_state_based_on_neighbor(G, node_id):
    ...
    if len(node_neighbors) >0:
        ...
        if np.random.random() < (infection_prob+ 0.05*proportion_of_infected_neighbors):
...
def initialize():
    ...
    mobile_nodes_in_reg2 = list(np.random.choice(all_region2_nodes_list, round(0.5*len(all_region2_nodes_list)), replace=False))
...
def update():
    ...
    crossing_nodes1= list(np.random.choice(mobile_nodes_in_reg1,round(0.05*len(mobile_nodes_in_reg1)), replace=False))
    crossing_nodes2= list(np.random.choice(mobile_nodes_in_reg2,round(0.05*len(mobile_nodes_in_reg2)), replace=False))
...
#             print("got here :1")
            ...
            if G.nodes[node_id]["days_since_recovery"]>=np.random.uniform(4,7):
                nextNetwork.nodes[node_id]["status"]="susceptible"
                #nextNetwork.nodes[node_id]["counter"]=0
                nextNetwork.nodes[node_id]["color"]="blue"
                nextNetwork.nodes[node_id]["days_since_recovery"]=0
            ...
            if G.nodes[node_id]["cut_off"]:
                ...
                if infected_neighbors:
                    ...
                    if random.random() < linkCuttingProb and G.nodes[node_id]["status"]=="susceptible":
                        ...
                        infect_neighbor_prop=len(infected_neighbors)/len(neighbors)
#                         if random.random() < (infection_prob + 0.05*infect_neighbor_prop):
#                             nextNetwork.nodes[node_id]["status"]="infected"
#                             nextNetwork.nodes[node_id]["color"]="red"
#                             nextNetwork.nodes[node_id]["counter"]+=1
#                             nextNetwork.nodes[node_id]["days_since_recovery"]=0
                        
                        if infect_neighbor_prop >= 0.7:
                            if G.nodes[node_id]["status"]=="recovered":
                                nextNetwork.nodes[node_id]["status"]="susceptible"
                                nextNetwork.nodes[node_id]["counter"]=0
                                nextNetwork.nodes[node_id]["color"]="blue"
                                nextNetwork.nodes[node_id]["days_since_recovery"]=0
